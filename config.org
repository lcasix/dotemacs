...
#+begin_comment
Ci vorrebbe una funzione che sfrutta projectile per fare la ricerca
dei tag sull'intero progetto.
#+end_comment
...
(use-package vue-mode
  ...
  (("C-M-f" . eslint-fix)))
#+end_src
L'/hook/ impostato è un /workaround/ per far funzionare l'indentazione
in =js-mode= (vedi la [[https://github.com/AdamNiederer/vue-mode/issues/74#issuecomment-577338222][issue]]).
...
** TODO Yasnippet                                                       :env:
#+begin_comment
Ci vorrebbe un /binding/ sensato per l'espansione di uno snippet su
una region e un bel riepilgo degli snippet disponibili per ogni mode,
perché c'è il rischio di non usarli mai perché non mi ricordo quali
sono.
#+end_comment
...
(define-abbrev-table 'global-abbrev-table '(
    ...
    ;; me
    ("8me" "" (lambda () (insert user-full-name)))
    ("8mail" "" (lambda () (insert user-mail-address)))

    ;; work
    ("8log" "" lc/activity-log)

    ;; misc
    ("4lorem" "" (lambda () (lorem-ipsum-insert-sentences)))
    ("8lorem" "" (lambda () (lorem-ipsum-insert-paragraphs)))
    ))

;; org mode
(define-abbrev-table 'org-mode-abbrev-table
  '(("8ref" "" lc/reference-list)
    ))
#+end_src
