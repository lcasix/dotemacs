...
** Setup di Ivy
Ivy è un frontend per il completamento dei nomi e non solo.
...
all'indirizzo del contatto:
#+begin_src emacs-lisp
(defun lc/contact-send-mail ()
  "Send a mail to the contact at point."
  (interactive)
...
(defun lc/contact-add-columns-property ()
  (interactive)
  (org-set-property "COLUMNS" "%25item %15phone %40email")
)
#+end_src
...
/Notmuch/ per la capacità di indicizzazione e la semplicità
dell'interfaccia.
#+begin_src emacs-lisp
(use-package notmuch
  :ensure t
...
installato sulla macchina. A beneficio dell'MTA viene impostato il
campo =from= nell'/envelope/, così per esempio si può decidere il
server SMTP in base all'indirizzo del mittente.
...
- [[shell:journalctl --user -u emacs -rq --no-pager][log completo]]
- [[shell:journalctl --user -u emacs -q --no-pager --since today][log di oggi]]
