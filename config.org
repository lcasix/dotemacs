...
[[https://github.com/bbatsov/crux][Crux]] contiene anche funzioni utili durante l'editing del codice:
#+begin_src emacs-lisp
(bind-keys :map prog-mode-map
           ("C-c d" . crux-duplicate-current-line-or-region)
           ("C-c M-d" . crux-duplicate-and-comment-current-line-or-region))
#+end_src
Al momento ne utilizzo un paio, utili nei flussi di "copia, incolla e
modifica":
...
template i file di nuova creazione. Quando si crea un nuovo file fra
quelli definiti con =define-auto-insert=, questo viene popolato dal
template indicato. Poiché questi template hanno la sintassi degli
snippet, dopo che =autoinsert= li ha copiati viene invocata la
funzione =lc/autoinsert-yas-expand=, la quale non fa altro che
espanderli. I template si trovano in =~/insert=, che è la locazione di
/default/, la quale può essere personalizzata impostando la variabile
