...
(use-package org-tempo
  ...
  (push '("N" . "annotation") org-structure-template-alist)
  )
#+end_src
Qui aggiungo le espansioni per i seguenti blocchi:
- un blocco =BEGIN_NOTE= che uso soprattutto per l'esportazione in EPUB
- un blocco =BEGIN_ANNOTATION= che uso in Latex per inserire nei
  documenti delle annotazioni che possono essere eliminate
  nell'esportazione
...
#+begin_comment
Per ottenere il link di Dropbox al calendario, per importarlo in
Google Calendar p.es., occorre trasformarlo come spiegatato qui:
...
(use-package magit
  ...
  (magit-add-section-hook 'magit-status-sections-hook
                          #'magit-insert-assume-unchanged-files 'append t))
#+end_src
La chiamata =magit-add-section-hook= aggiunge l'elenco dei file
"assumed unchanged" allo /status buffer/.
