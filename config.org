...
Tuttavia, per fare in modo che il processo Emacs erediti l'ambiente
completo dell'utente, preferisco che ad avviarlo sia il /desktop
manager/. In KDE Plasma basta collocare uno script come questo in
[[~/.config/autostart/]]:
#+begin_src sh :eval no :tangle ~/.config/autostart/start-emacs-sever :tangle-mode (identity #o544) :shebang "#!/bin/sh"
